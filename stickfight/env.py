...
SCREEN_WIDTH = 1000  # enlarged arena width
SCREEN_HEIGHT = 700  # enlarged arena height
PPM = 1.0  # pixels per meter scaling (keep 1 for simplicity)
PHYSICS_STEPS_PER_ENV_STEP = 6
TIME_STEP = 1.0 / 120.0  # physics tick
MAX_HP = 100.0
...
class StickmanFightEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, dummy_policy: str = "random", role_swap: bool = True, opponent_policy=None):
        ...
        left_wall = pymunk.Segment(wall_body, (0, 50), (0, SCREEN_HEIGHT - 10), 5)
    ...
    def _make_damage_callback(self, attacker_index: int, victim_index: int):
        def _cb(arbiter: pymunk.Arbiter, space: pymunk.Space, data):
            ...
            dir_vec = v1 - v2
            if dir_vec.length > 0:
                impulse = dir_vec.normalized() * damage * KNOCKBACK_SCALE
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None):
        ...
        for s in list(self.space.shapes):
            if s.collision_type != GROUND_COLLISION_TYPE:
                self.space.remove(s)
        ...
        self.agent_a = Stickman(self.space, (SCREEN_WIDTH * 0.35, 200), agent_index=0)
        self.agent_b = Stickman(self.space, (SCREEN_WIDTH * 0.65, 200), agent_index=1)
        self.hp_a = MAX_HP
        self.hp_b = MAX_HP
        self.steps = 0
        self.damage_dealt = [0.0, 0.0]
    ...
    def step(self, action: np.ndarray):
        ...
        for _ in range(PHYSICS_STEPS_PER_ENV_STEP):
            # Apply damping to all dynamic bodies prior to physics step
            for body in self.space.bodies:
                if body.body_type != pymunk.Body.DYNAMIC:
                    continue
                body.velocity = body.velocity * DAMP_LINEAR
                body.angular_velocity = body.angular_velocity * DAMP_ANGULAR
        ...
        approach_reward = (prev_dist - new_dist) * APPROACH_REWARD_SCALE  # positive if moved closer
        self._prev_approach_dist = new_dist
        reward = self._compute_reward() + approach_reward
        terminated = self._is_terminated()
        truncated = self.steps >= EPISODE_LENGTH
        obs = self._get_observation()
        info = {
            'hp_a': self.hp_a,
            'hp_b': self.hp_b,
    ...
    def _normalize_pos(self, x: float, y: float) -> Tuple[float, float]:
        return x / SCREEN_WIDTH * 2 - 1, y / SCREEN_HEIGHT * 2 - 1
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        hp_self_norm = (self.hp_a if self.self_index == 0 else self.hp_b) / MAX_HP
        hp_opp_norm = (self.hp_b if self.self_index == 0 else self.hp_a) / MAX_HP
    ...
    def _get_opponent_observation(self) -> np.ndarray:
        ...
        for name in ['l_hand', 'r_hand', 'l_foot', 'r_foot']:
            ...
            rel = (pt[0] - com_ax, pt[1] - com_ay)
            limb_rel_norms.extend(self._normalize_pos(*rel))
        hp_self_norm = (self.hp_a if opp_index == 0 else self.hp_b) / MAX_HP
        hp_opp_norm = (self.hp_b if opp_index == 0 else self.hp_a) / MAX_HP
        return np.array([
            *angles, *ang_vels,
            torso_angle, vx, vy, feet_flag,
            *rel_head_n, *rel_torso_n,
    ...
    def render(self):
        ...
        bar_h = 15
        # Agent A
        a_ratio = self.hp_a / MAX_HP
        b_ratio = self.hp_b / MAX_HP
        ...
        pygame.draw.rect(self.screen, (80, 80, 80), pygame.Rect(SCREEN_WIDTH - 40 - bar_w, 20, bar_w, bar_h))
        pygame.draw.rect(self.screen, (200, 60, 40), pygame.Rect(SCREEN_WIDTH - 40 - bar_w, 20, int(bar_w * b_ratio), bar_h))
